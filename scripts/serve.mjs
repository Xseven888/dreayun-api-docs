...
// Project-local brand/API localization. The forensic mirror stays pristine;
// only the rebuild response layer applies this registered adaptation. The
// branding endpoint contains both locale names, while ordinary content uses
// the request locale to choose the visible brand.
const BRAND_REWRITE = SIDE === "rebuild";
const BRAND_API_HOST = "api.openlux.ai";
const BRAND_API_HOST_REPLACEMENT = "api.dreayun.com";
let brandRewriteLogged = false;
...
// OpenLux is a history-fallback SPA: the live origin serves its 984-byte
// entry shell for deep client routes. Keep the mirror bytes untouched and
// resolve unknown extensionless paths to the matching captured locale shell.
...
function rewrite(text, ext, locale = "zh") {
  ...
  if (ext === ".html" && hasFlight(text)) {
    const done = rewriteFlight(text, (t) => rewriteText(t, ext, locale));
    if (done !== null) return done;
  }
  return rewriteText(text, ext, locale);
}

function rewriteText(text, ext, locale = "zh") {
  ...
  // Brand/API localization is scoped to the rebuild side. Replacing the host
  // itself covers absolute, protocol-relative, JSON-escaped, and unicode-
  // escaped URL spellings without changing paths or API examples.
  if (BRAND_REWRITE) {
    const before = text;
    text = text.replace(/api\.openlux\.ai/gi, BRAND_API_HOST_REPLACEMENT);
    // Keep both fields correct in the same branding response. The generic
    // locale-aware replacement below handles ordinary page/API/tutorial text.
    text = text
      .replace(/([\"']system_name[\"']\s*:\s*[\"'])OpenLux([\"'])/g, "$1即云API$2")
      .replace(/([\"']server_name_en[\"']\s*:\s*[\"'])OpenLux([\"'])/g, "$1DreaYunAPI$2");
    // Only translate remaining visible/content occurrences. The structured
    // branding fields above are already exact and must not be overwritten by
    // the page locale (both fields are returned together).
    text = text.replace(/OpenLux/g, locale === "en" ? "DreaYunAPI" : "即云API");
    // The shared bundle is requested from /assets/ for both locales, so the
    // asset request itself has no /en path and cannot be localized by the
    // server request locale. Patch the two direct branding fallbacks to make
    // their choice at runtime from the actual document path.
    if (ext === ".js" || ext === ".mjs") {
      text = text.replace(
        /document\.title=n\.system_name/g,
        'document.title=n.is_global_agent&&window.location.pathname.startsWith("/en")?n.server_name_en:n.system_name',
      );
      text = text.replace(
        /const U=\(o==null\?void 0:o\.system_name\)\?\?""/g,
        'const U=(o==null?void 0:(o.is_global_agent&&window.location.pathname.startsWith("/en")?o.server_name_en:o.system_name))??""',
      );
    }
    if (text !== before && !brandRewriteLogged) {
      brandRewriteLogged = true;
      console.log(`  [brand] rebuild localization: ${BRAND_API_HOST} -> ${BRAND_API_HOST_REPLACEMENT}; zh=即云API; en=DreaYunAPI`);
    }
  }
...
function requestLocale(url) {
  ...
  if (explicit) return /^en(?:-|$)/i.test(explicit) ? "en" : "zh";
...
export async function handleRequest(req, res) {
  try {
    ...
    if (!hit && SPA_FALLBACK && !path.extname(url.pathname) && !url.pathname.startsWith('/api/') && !url.pathname.startsWith('/assets/') && !url.pathname.startsWith('/ext/')) {
      const localeShell = url.pathname === '/en' || url.pathname.startsWith('/en/') ? '/en/index.html' : '/index.html';
      const fallback = await resolveFile(localeShell, '');
      if (fallback) {
        console.log("[spa-fallback] " + url.pathname + " -> " + localeShell);
        const fallbackExt = path.extname(fallback.file).toLowerCase();
        const fallbackHeaders = {
          'content-type': RECORDED_TYPE.get(fallback.file) || MIME[fallbackExt] || 'text/html; charset=utf-8',
          'cache-control': 'no-cache',
          'access-control-allow-origin': '*',
        };
        let html = await fsp.readFile(fallback.file, 'utf8');
        if (RESPONSE_REWRITE) html = rewrite(html, '.html', requestLocale(url));
        const body = Buffer.from(html, 'utf8');
        res.writeHead(200, { ...fallbackHeaders, 'content-length': body.length });
        return res.end(body);
      }
    }
    if (!hit) {
      ...
      if (tpl && !url.pathname.startsWith("/ext/")) {
        const html = rewrite(await fsp.readFile(tpl.file, "utf8"), ".html", requestLocale(url));
    ...
    if (((TEXT_REWRITE.has(ext) || textByType) && RESPONSE_REWRITE) || wantsProbe) {
      let text = await fsp.readFile(hit.file, "utf8");
      if (RESPONSE_REWRITE) text = rewrite(text, rwExt, requestLocale(url));
